...
import { get_user_info, update_profile } from '../constants/Api';
...
export default class MyProfile extends React.Component {
    ...
    async componentDidMount(){
        ...
        .then(async (response) => {
            ...
            if(response.data) {
                ...
                this.setState({car_picture: 'http://149.28.78.240:5010/' + response.data.car_picture})
                this.setState({license_front: 'http://149.28.78.240:5010/' + response.data.license_front})
                this.setState({license_back: 'http://149.28.78.240:5010/' + response.data.license_back})
            }
        })
        .catch((error) => {
            this.setState({loaded: true});
    ...
    async signup() {
        ...
        if(valid) {
            ...
            await update_profile(info['id'], this.state.user, this.state.password, this.state.email, this.state.phone, this.state.country, this.state.countrycode, this.state.kind, this.state.car_color, this.state.plate_no, this.state.car_picture, this.state.license_front, this.state.license_back)
            .then(async (response) => {
                this.setState({loaded: true});
            ...
            <SafeAreaView style={styles.container}>
                ...
                <KeyboardAwareScrollView resetScrollToCoords={{ x: 0, y: 0 }} scrollEnabled={true} style={{flex: 1}}>
                    <View style={{padding: 20}}>
                        <Text style={{fontWeight: 'bold', fontSize: 20, paddingBottom: 20}}>My Profile</Text>
                        ...
                        
                        {
                            ...
                            <View>
                                ...
                                <Text>Face Picture</Text>
                                {
                                    this.state.car_picture ?
                                    <Image source={{uri: this.state.car_picture}} style={{width: 150, height: 150, marginTop: 5}} resizeMode="contain" resizeMethod="resize" />
                                    :
                                    null
                                }
                                ...
                                <Text>License Front</Text>
                                {
                                    ...
                                    <Image source={{uri: this.state.license_front}} style={{width: 150, height: 150, marginTop: 5}} resizeMode="contain" resizeMethod="resize" />
                                    :
                                    null
                                }
                                ...
                                <Text>License Back</Text>
                                {
                                    ...
                                    <Image source={{uri: this.state.license_back}} style={{width: 150, height: 150, marginTop: 5}} resizeMode="contain" resizeMethod="resize" />
                                    :
                                    null
                                }
                            </View>
