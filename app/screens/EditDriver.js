...
import { get_user_info, update_profile, client_signup } from '../constants/Api';
...
export default class EditDriver extends React.Component {
    ...
    async componentDidMount(){
        if(this.props.driver_id) {
            ...
            await get_user_info(this.props.driver_id)
            .then(async (response) => {
                ...
                if(response.data) {
                    ...
                    this.setState({countrycode: response.data.phone_code})
                    this.setState({type: response.data.type})
                    this.setState({kind: response.data.kind})
                    this.setState({car_color: response.data.car_color})
                    this.setState({plate_no: response.data.plate_no})
                    this.setState({car_picture: response.data.car_picture})
                    this.setState({license_front: response.data.license_front})
                    this.setState({license_back: response.data.license_back})
                }
            })
            .catch((error) => {
                this.setState({loaded: true});
    ...
    async signup() {
        ...
        if(valid) {
            this.setState({loaded: false});
            if(this.props.driver_id)
                await update_profile(this.props.driver_id, this.state.user, this.state.password, this.state.email, this.state.phone, this.state.country, this.state.countrycode, this.state.kind, this.state.car_color, this.state.plate_no, this.state.car_picture, this.state.license_front, this.state.license_back)
                .then(async (response) => {
                    this.setState({loaded: true});
                ...
                await client_signup(this.state.user, this.state.password, this.state.email, this.state.phone, this.state.country, 0, this.state.countrycode, this.state.kind, this.state.car_color, this.state.plate_no, this.state.car_picture, this.state.license_front, this.state.license_back)
                .then(async (response) => {
                    this.setState({loaded: true});
                    if(response.data)
                        Actions.reset("admin")
            ...
            <SafeAreaView style={styles.container}>
                ...
                <KeyboardAwareScrollView resetScrollToCoords={{ x: 0, y: 0 }} scrollEnabled={true} style={{flex: 1}}>
                    <View style={{padding: 20}}>
                        <Text style={{fontWeight: 'bold', fontSize: 20, paddingBottom: 20}}>Driver Profile</Text>
                        ...
                        <View style={{flexDirection: 'row', flex: 1, borderColor: '#ABAAAC', borderWidth: 1, borderRadius: 6,marginBottom: 20, backgroundColor: '#F7F8F9'}}>
                            ...
                            <TextInput
                                style={this.state.phoneError ? [styles.input, styles.error,{borderWidth: 0, borderRadius: 0, marginBottom: 0}] : [styles.input,{borderWidth: 0, borderRadius: 0, marginBottom: 0}]}
                        ...
                        />
                            <View>
                                ...
                                {
                                    this.state.car_picture && this.state.car_picture.uri ?
                                    ...
                                    <Image source={{uri: 'http://149.28.78.240:5010/'+this.state.car_picture}} style={{width: 150, height: 150, marginTop: 5}} resizeMode="center" resizeMethod="resize" />
                                    :
                                    null
                                }
                                ...
                                {
                                    this.state.license_front && this.state.license_front.uri ?
                                    ...
                                    <Image source={{uri: 'http://149.28.78.240:5010/'+this.state.license_front}} style={{width: 150, height: 150, marginTop: 5}} resizeMode="center" resizeMethod="resize" />
                                    :
                                    null
                                }
                                ...
                                {
                                    this.state.license_back && this.state.license_back.uri ?
                                    ...
                                    <Image source={{uri: 'http://149.28.78.240:5010/'+this.state.license_back}} style={{width: 150, height: 150, marginTop: 5}} resizeMode="center" resizeMethod="resize" />
                                    :
                                    null
                                }
                        ...
                        <TouchableOpacity style={{alignItems: 'center', marginTop: 15}} onPress={() => this.signup()}>
                            <View style={{backgroundColor: '#dab844', paddingVertical: 10, width: 150, borderRadius: 5, alignItems: 'center', flex: 1}}>
                                <Text style={{color: 'white', textAlign: 'center', fontSize: 20}}>{this.props.driver_id ? 'Update' : 'Add driver'}</Text>
                            </View>
                        </TouchableOpacity>
                    </View>
